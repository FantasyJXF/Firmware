 ...
 * @file rtk.cpp
 * @author CazadorH
 * @email: xiaor@ramytech.com
 * Driver for the RTK on a serial port
 */
 ...
 * Driver 'main' command.
 */
extern "C" __EXPORT int reachrtk_main(int argc, char *argv[]);
...
int GPS::init()
{
	...
	_task = px4_task_spawn_cmd("rtk", SCHED_DEFAULT,
...
int
reachrtk_main(int argc, char *argv[])
{
	/* set to default */
	const char *device_name = RTK_DEFAULT_UART_PORT;
	bool fake_gps = false;
