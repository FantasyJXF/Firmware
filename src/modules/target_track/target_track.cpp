...
int target_track_thread_main(int argc, char *argv[]){
	...
	//订阅相关主题
	int sensor_distance_sub = orb_subscribe(ORB_ID(distance_sensor));
	int global_position_sub = orb_subscribe(ORB_ID(vehicle_global_position));
	int target_basic_sub = orb_subscribe(ORB_ID(target_basic));
	orb_advert_t target_track_pub = orb_advertise(ORB_ID(follow_target),&target_track);
	...
	while(!thread_should_exit)
	{
		...
		int pret = px4_poll(&fds[0], 1 , 100);//阻塞100ms，10HZ
		...
		//if(pret>0){//轮询成功
			...
			/*PX4_WARN("The degree here is %d",target_basic.degree);*/
			...
			orb_check(sensor_distance_sub,&updated);
			if(updated){
				orb_copy(ORB_ID(distance_sensor),sensor_distance_sub,&target_distance);
						}
			...
			/*PX4_INFO("Vector:\t%8.4f\t%8.4f\t%8.4f",
					 (double)target_vector_basic(0),
					 (double)target_vector_basic(1),
					 (double)target_vector_basic(2));*/
...
int target_track_main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "start")) {
		...
		daemon_task = px4_task_spawn_cmd("target_track",
							SCHED_DEFAULT, 
